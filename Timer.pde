class Timer {
  ...
  Timer() {
    ...
    youWonImg = loadImage("youwon.png"); // Replace with actual image path
    gameOverImg = loadImage("youlost.png"); // Replace with actual image path
  }
  ...
  void display() {
    ...
    if (score >= 60) {
      //Display the "You Won" image shifted to the right
      image(youWonImg, width - youWonImg.width , height / 2 - youWonImg.height / 2);
    } else {
      //Display the "Game Over" image shifted to the right
